...
<main class="h-full pb-16 overflow-y-auto">
    <br>
    <form action="{{route('admin.comment.update', $comments->id)}}" method="post" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="container px-6 mx-auto grid">
        <label class="block text-sm">
            <span class="text-gray-700 dark:text-gray-400">Comment</span>
            <input type="comment" name="comment" id="Comment" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
            placeholder="Comment">
        <br>
        </label>
        {{-- <label class="block text-sm">
